...
function buildMeshSurface(chunkData, cx, cz, neighborCache) {
    ...
    for (let x = 0; x < CHUNK_SIZE; x++) {
        for (let z = 0; z < CHUNK_SIZE; z++) {
            ...
            // Need all 4 corners to form a proper sloped quad
            ...
            // Choose the block for this surface from the highest corner
            let pickY = h00, pickX = x, pickZ = z;
            if (h10 >= pickY) { pickY = h10; pickX = x + 1; pickZ = z; }
            if (h01 >= pickY) { pickY = h01; pickX = x; pickZ = z + 1; }
            if (h11 >= pickY) { pickY = h11; pickX = x + 1; pickZ = z + 1; }
            const blockId = getBlockAt(chunkData, cx, cz, pickX, pickY, pickZ, neighborCache);
            if (!blockId) continue;
