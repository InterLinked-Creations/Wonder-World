...
function buildChunkGeometry(chunkData, cx, cz) {
            ...
            for (let y = 0; y <= topY; y++) {
                ...
                for (let f = 0; f < faces.length; f++) {
                    ...
                    if (hasFaceSpecificTexture || hasDefaultTexture) {
                        ...
                        const nxn = face.dir[0], nyn = face.dir[1], nzn = face.dir[2];
                        for (let i = 0; i < 4; i++) {
    ...
    // Choose smallest suitable index type to reduce memory/transfer
...
self.onmessage = function(e) {
    ...
    // One-time initialization to avoid resending constants for every message
    ...
    // Back-compat: if older main thread still sends constants, accept them once
    if (constants && (CHUNK_SIZE === undefined || WORLD_HEIGHT === undefined)) {
        CHUNK_SIZE = constants.CHUNK_SIZE;
        WORLD_HEIGHT = constants.WORLD_HEIGHT;
        BLOCK_TYPES = constants.BLOCK_TYPES;
        blockColors = constants.blockColors;
    ...
    if (!chunkData) {
        ...
        // Prefer fast builder when heightMap is available
        const geometryData = (chunkData.heightMap)
            ? buildChunkGeometryFast(chunkData, cx, cz)
            : buildChunkGeometry(chunkData, cx, cz);
    ...
    if (requestGeometry) {
        ...
        // Build geometry for existing chunk (full builder for updates)
